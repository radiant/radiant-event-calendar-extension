module Admin::CalendarHelper
  def self.included(base)
    ...
    base.module_eval do
      ...
      # Pulled ungraciusly from the calendar_helper plugin by topfunky
      # and adapted to suit radiant context and do the right thing with event_calendar events
      #...
      ...
      def calendar_month(options = {}, &block)
        ...
        firstday = Date.civil(options[:year], options[:month], 1)
        lastday = Date.civil(options[:year], options[:month], -1)
        firstshown = beginning_of_week(firstday, first_weekday)
        lastshown = beginning_of_week(lastday + 7, first_weekday) - 1
        previous = firstday - 1.day
        following = lastday + 1.day
        ...
        firstshown.upto(lastshown) do |cur|
          cell_text, cell_attrs = block.call(cur)
          cell_text  ||= cur.mday
          cell_attrs ||= {}
          cell_attrs[:class] ||= options[:day_class]
          cell_attrs[:class] += " weekend_day" if weekend?(cur)
          cell_attrs[:class] += " today" if today?(cur)
          cell_attrs[:class] += " weekend_today" if weekend?(cur) && today?(cur)
          cell_attrs[:class] += " other_month" if cur.month != options[:month]
      ...
      def calendar_day_with_events(day, events=[])
        ...
        if today.any?
          ...
          text << "</ul>"
          text << "</div>"
          [text.to_s, {:class => weekend?(day) ? 'eventful_weekend' : 'eventful'}]
        else
          text = %{<div class="no_events">#{day.mday}</div}
          [text, {}]
        end
      end
